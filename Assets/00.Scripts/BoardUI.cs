...
public class BoardUI : MonoBehaviour
{
    ...
    private void Start()
    {
        int size = maker.texture.width;
        GenerateHintText(size);
        SetHintText(size);
    }

    private void GenerateHintText(int size)
    {
        int textSize = 600 / size;
    ...
    private void SetHintText(int size)
    {
        for(int i = 0; i < size; i++)
        {
            // rowHintTxt[i].text = maker.inputRowHint[i];
            // colHintTxt[i].text = maker.inputColHint[i].Replace(',','\n');
        }
    }
}
